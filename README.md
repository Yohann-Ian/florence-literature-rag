...
It sits at the intersection of three things I care about: building **AI/ML systems** properly (evaluation, observability, deployment - not just a notebook), the **engineering discipline** of measuring before optimising, and **literature** as a domain that genuinely stresses the limits of current retrieval methods. The hard problems here - grounding against a corpus the model already knows, retrieving meaning that lives in subtext - are exactly the ones a generic technical-docs RAG never has to confront.
